...
do
    echo "============================================================"
    echo "Processing non-default config ${config}"
    config_name=$(basename ${config%%.conf})
    mkdir -p .test/run-conf/${config_name}/out
    tox -e compare-xml-config -- --conf ${config} test -o .test/run-conf/${config_name}/out/ .test/run-conf/config
    echo "------------------------------------------------------------"
done
...
do
    echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
    echo "Comparing config ${config}"
    config_name=$(basename ${config%%.conf})
    CHANGED=0
    for x in `(cd .test/run-conf/default/out/ && find -type f)`
    do
        differences=$(diff -u .test/run-conf/default/out/${x} .test/run-conf/${config_name}/out/${x} 2>&1)
        if [[ $? -ne 0 ]]
        then
            CHANGED=1
            echo "============================================================"
    ...
    if [ "$CHANGED" -eq "0" ]
    then
        echo "No differences between default and ${config_name} configs"
    fi
done
exit 0
