...
class YamlParser(object):
    ...
    def applyDefaults(self, data, override_dict=None):
        ...
        if self.merge_defaults:
            newdata.update(self.render_macro(defaults))
            self.deepUpdate(newdata, self.render_macro(data))
        else:
    ...
    def render_macro(self, data):
        data_copy = copy.deepcopy(data)
        for module in self.registry.modules:
            if hasattr(module, 'render_macro'):
                data_copy.update(module.render_macro(self, data_copy))
        return data_copy
