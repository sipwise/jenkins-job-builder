...
class IncludeJinja2(J2BaseYamlObject):
    ...
    def __init__(self, jjb_config, loader, rel_path_template):
        super().__init__(jjb_config, loader)
        self._rel_path_template = rel_path_template

    def expand(self, expander, params):
        rel_path = self._formatter.format(self._rel_path_template, **params)
...
class IncludeRawBase(BaseYamlObject):
    @classmethod
    def from_yaml(cls, loader, node):
        if isinstance(node, yaml.ScalarNode):
            path_list = [loader.construct_yaml_str(node)]
        elif isinstance(node, yaml.SequenceNode):
            path_list = loader.construct_sequence(node)
        else:
