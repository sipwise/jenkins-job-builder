    ...
    for project_def in data:
        ...
        if 'bool-parameters' in project_def:
            ...
            for bool_param in project_def['bool-parameters']:
                param_name = bool_param['name']
                param_value = bool_param.get('value', False)

                param = XML.SubElement(configs,
                                       'hudson.plugins.parameterizedtrigger.'
                                       'BooleanParameterConfig')
                name = XML.SubElement(param, 'name')
                name.text = str(param_name)
                name = XML.SubElement(param, 'value')
                name.text = str(param_value).lower()
