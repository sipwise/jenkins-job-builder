...
def trigger_remote(parser, xml_parent, data):
    ...
    XML.SubElement(triggerr, 'remoteJenkinsName').text = \
        data.get('remote-jenkins-name')
    ...
    for attribute in ['job', 'remote-jenkins-name']:
        if attribute not in data or data[attribute] == '':
            raise ("Required attribute, {0}, missing - skipping"
    ...
    XML.SubElement(triggerr, 'pollInterval').text = \
        str(data.get('poll-interval', 10))
    XML.SubElement(triggerr, 'connectionRetryLimit').text = \
        str(data.get('connection-retry-limit', 5))

    XML.SubElement(triggerr, 'preventRemoteBuildQueue').text = \
        str(data.get('prevent-remote-build-queue', False)).lower()

    XML.SubElement(triggerr, 'blockBuildUntilComplete').text = \
        str(data.get('block', True)).lower()
    ...
    if 'property-file' in data and data['property-file'] != '':
        ...
        XML.SubElement(triggerr, 'parameterFile').text = \
            data.get('property-file')
    else:
