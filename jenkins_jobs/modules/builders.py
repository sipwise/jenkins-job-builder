...
def saltstack(parser, xml_parent, data):
    ...
    if data.get('authtype', 'pam') not in supported_auth_types:
        raise InvalidAttributeError('authtype', data.get('authtype'),
                                    supported_auth_types)

    supported_target_types = ['glob', 'pcre', 'list', 'grain', 'pillar',
                              'nodegroup', 'range', 'compound']
    if data.get('targettype', 'glob') not in supported_target_types:
        raise InvalidAttributeError('targettype', data.get('targettype'),
                                    supported_target_types)

    supported_client_interfaces = ['local', 'local-batch', 'runner']
    clientInterface = data.get('clientinterface', 'local')
    if clientInterface not in supported_client_interfaces:
        raise InvalidAttributeError('clientinterface', clientInterface,
                                    supported_client_interfaces)

    mapping = [
        ('authtype', 'authtype', 'pam'),
        ('target', 'target', ''),
    ...
    for emt in sorted(ci_config.keys()):
        ...
        if emt != 'usePillar':
            ...
            for pemt in sorted(pillar_cfg.keys()):
                ...
                XML.SubElement(pillarEntry, 'string').text = pillar_cfg[pemt]
