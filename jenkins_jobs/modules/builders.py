...
def http_request(parser, xml_parent, data):
    ...
    if 'mode' in data and data['mode'] not in valid_modes:
        raise InvalidAttributeError('mode', data['mode'], valid_modes)
    if 'content-type' in data and data['content-type'] not in valid_types:
        raise InvalidAttributeError('content-type',
                                    data['content-type'],
                                    valid_types)
    if 'accept-type' in data and data['accept-type'] not in valid_types:
