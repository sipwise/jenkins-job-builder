...
def ansible(parser, xml_parent, data):
    ...
    :arg str tags: only run plays and tasks tagged with these values. (optional)
    ...
    .. literalinclude:: ../../tests/builders/fixtures/ansible001.yaml
       :language: yaml
    """
    ansible = XML.SubElement(xml_parent,
    ...
    if inventory_file is not None:
        ...
        XML.SubElement(inventory, 'content').text = data.get('inventory-content')
    ...
    for k, v in data.get('extra-vars', {}).items():
        extraVar = XML.SubElement(extraVars, 'org.jenkinsci.plugins.ansible.ExtraVar')
