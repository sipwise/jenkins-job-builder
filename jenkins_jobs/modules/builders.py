...
def system_groovy(parser, xml_parent, data):
    ...
    :arg command: Groovy command to run.
      (Alternative: you can choice a script file instead)
    :arg bindings: Define variable bindings (in the properties file format).
      Specified variables can be addressed from the script.
    :arg class-path: Specify script classpath here. Each line is one class
      path item.
    ...
    scriptSource = XML.SubElement(sysgroovy, "scriptSource")
    if 'command' in data:
        command = XML.SubElement(scriptSource, 'command')
        command.text = str(data['command'])
        scriptSource.set('class', 'hudson.plugins.groovy.StringScriptSource')
    ...
    if 'bindings' in data:
        XML.SubElement(sysgroovy, 'bindings').text = str(data['bindings'])
    if 'class-path' in data:
        XML.SubElement(sysgroovy, 'classpath').text = str(data['class-path'])
