...
def sonatype_clm(parser, xml_parent, data):
    ...
    :arg str username: Username on the Sonatype CLM server (required)
    :arg str password: Password on the Sonatype CLM server (required)
    ...
    clm_value = data.get('value', SUPPORTED_VALUES)
    if clm_value not in SUPPORTED_VALUES:
        raise InvalidAttributeError('value',
                                    clm_value,
                                    SUPPORTED_VALUES)
    SUPPORTED_STAGES = ['build', 'stage-release', 'release', 'operate']
    clm_stage = data.get('stage', SUPPORTED_STAGES[0])
    if clm_stage not in SUPPORTED_STAGES:
