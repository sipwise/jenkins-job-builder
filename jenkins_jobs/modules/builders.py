...
import jenkins_jobs.modules.hudson_model as hudson_model
...
def trigger_builds(parser, xml_parent, data):
    ...
    for project_def in data:
        ...
        if(block):
            ...
            XML.SubElement(bsft, 'name').text = hudson_model.FAILURE['name']
            XML.SubElement(bsft, 'ordinal').text = hudson_model.FAILURE['ordinal']
            XML.SubElement(bsft, 'color').text = hudson_model.FAILURE['color']
            ut = XML.SubElement(block, 'unstableThreshold')
            XML.SubElement(ut, 'name').text = hudson_model.UNSTABLE['name']
            XML.SubElement(ut, 'ordinal').text = hudson_model.UNSTABLE['ordinal']
            XML.SubElement(ut, 'color').text = hudson_model.UNSTABLE['color']
            ft = XML.SubElement(block, 'failureThreshold')
            XML.SubElement(ft, 'name').text = hudson_model.FAILURE['name']
            XML.SubElement(ft, 'ordinal').text = hudson_model.FAILURE['ordinal']
            XML.SubElement(ft, 'color').text = hudson_model.FAILURE['color']
...
def conditional_step(parser, xml_parent, data):
    ...
    current-status     Run the build step if the current build status is
                       within the configured range
                         ...
                         :condition-worst: Worst status (accepted values are: success, unstable,
                         failure, not_build, aborted)
                         :condition-best: Best status (accepted values are: success, unstable,
                         failure, not_build, aborted)
    ...
    def build_condition(cdata):
        ...
        if kind == "always":
            ...
            if not wr_name in hudson_model.THRESHOLDS:
            ...
            XML.SubElement(wr, "completeBuild").text = 'true' if wr_threshold['complete'] else 'false'
            ...
            XML.SubElement(br, "completeBuild").text = 'true' if br_threshold['complete'] else 'false'
        elif kind == "shell":
            ctag.set('class',
