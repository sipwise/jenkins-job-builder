...
def shining_panda(parser, xml_parent, data):
    ...
    try:
        ...
        raise MissingAttributeError(
            'build-environment', 'builders.shining-panda')

    if buildenv not in envs:
        raise InvalidAttributeError(
            'build-environment', buildenv, 'builders.shining-panda', envs)
    ...
    if nature not in naturetuple:
        raise InvalidAttributeError(
            'nature', nature, 'builders.shining-panda', naturetuple)
...
def managed_script(parser, xml_parent, data):
    ...
    if step_type == 'script':
        ...
        raise InvalidAttributeError(
            'type', step_type, 'builders.managed-script', ['script', 'batch'])
    ms = XML.SubElement(xml_parent,
                        'org.jenkinsci.plugins.managedscripts.' + step)
    try:
        script_id = data['script-id']
    except KeyError:
        raise MissingAttributeError('script-id', 'builders.managed-script')
...
def cmake(parser, xml_parent, data):
    ...
    try:
        ...
        raise MissingAttributeError('source-dir', 'builders.cmake')
...
def sonatype_clm(parser, xml_parent, data):
    ...
    :arg str stage: Controls the stage the policy evaluation will be run
        against on the CLM server. (Default: build)
    ...
    if stage not in STAGES:
        logger.error("Invalid stage, must be one of %s" % STAGES)
