...
def trigger_builds(parser, xml_parent, data):
    ...
    :arg list parameter-factories: list of parameter factories
      ...
      :Factory: * **factory** (`str`) **filebuild** -- For every property file, invoke one build
                ...
                * **no-files-found-action** (`str`) -- Action to perform when no files found  (optional) ['FAIL', 'SKIP', 'NOPARMS']
                  (default 'SKIP')
      ...
      :Factory: * **factory** (`str`) **counterbuild** -- Invoke i=0...N builds
                ...
                * **parameters** (`str`) -- KEY=value pairs, one per line
                  (default '')
                * **validation-fail** (`str`) -- Action to perform when stepping validation fails  (optional) ['FAIL', 'SKIP', 'NOPARMS']
                  (default 'FAIL')
    ...
    for project_def in data:
        ...
        if 'property-file' in project_def:
            ...
            failTriggerOnMissing = XML.SubElement(params, 'failTriggerOnMissing')
        ...
        if 'parameter-factories' in project_def:
            ...
            for factory in project_def['parameter-factories']:
                ...
                if factory['factory'] == 'filebuild':
                    params = XML.SubElement(fconfigs,
                                            'hudson.plugins.parameterizedtrigger.'
                ...
                if (factory['factory'] == 'filebuild') or (factory['factory'] == 'binaryfile'):
                    filePattern = XML.SubElement(params, 'filePattern')
                    filePattern.text = factory['file-pattern']
                    noFilesFoundAction = XML.SubElement(params, 'noFilesFoundAction')
