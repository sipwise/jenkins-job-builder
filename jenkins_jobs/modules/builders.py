...
def trigger_builds(parser, xml_parent, data):
    ...
    :arg dict block-thresholds: Fail builds and/or mark as failed or unstable
      based on thresholds. (optional)

      * **build-step-failure-threshold** (`str`) (optional) \
                           ['never', 'SUCCESS', 'UNSTABLE', 'FAILURE'] \
                           (default: 'FAILURE')
      * **unstable-threshold** (`srt`) (optional) \
                           ['never', 'SUCCESS', 'UNSTABLE', 'FAILURE'] \
                           (default: 'UNSTABLE')
      * **failure-threshold** (`srt`) (optional) \
                           ['never', 'SUCCESS', 'UNSTABLE', 'FAILURE'] \
                           (default: 'FAILURE')
    ...
    for project_def in data:
        ...
        if block:
            ...
            for threshold in supported_thresholds:
                threshold_value = threshold[2]
                thrsh = project_def.get('block-thresholds', False)
                if thrsh:
                    threshold_value = thrsh.get(threshold[0], threshold[2])
                if str(threshold_value).lower() == \
                        supported_threshold_values[0]:
                    continue
                if str(threshold_value).upper() not in \
                        supported_threshold_values:
                    raise JenkinsJobsException(
                        "threshold value must be one of (%s)" %
                        ", ".join(supported_threshold_values))
                th = XML.SubElement(block, threshold[1])
                XML.SubElement(th, 'name').text = \
                    hudson_model.THRESHOLDS[
                        str(threshold_value).upper()]['name']
                XML.SubElement(th, 'ordinal').text = \
                    hudson_model.THRESHOLDS[
                        str(threshold_value).upper()]['ordinal']
                XML.SubElement(th, 'color').text = \
                    hudson_model.THRESHOLDS[
                        str(threshold_value).upper()]['color']
