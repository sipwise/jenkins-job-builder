...
def multijob(parser, xml_parent, data):
    ...
    for project in data.get('projects', []):
        ...
        currParams = str(data.get('current-parameters', False)).lower()
        XML.SubElement(phaseJob, 'currParams').text = currParams

        # I'm not actually sure what this option does!
        exposedSCM = str(data.get('exposed-scm', False)).lower()
        XML.SubElement(phaseJob, 'exposedSCM').text = exposedSCM

        # Pass through other params
        configs = XML.SubElement(phaseJob, 'configs')

        # Git Revision
        if data.get('git-revision', False):
        ...
        # Properties File
        properties_file = data.get('property-file', False)
        if properties_file:
            param = XML.SubElement(configs,
                                   'hudson.plugins.parameterizedtrigger.'
                                   'FileBuildParameters')
        ...
        # Predefined Parameters
        predefined_parameters = data.get('predefined-parameters', False)
