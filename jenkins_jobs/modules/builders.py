...
def dsl(parser, xml_parent, data):
    ...
    supportedJobActions = ['IGNORE', 'DELETE']
    removedViewAction = data.get('removed-view-action', 'IGNORE')
    if removedViewAction not in supportedJobActions:
        raise JenkinsJobsException("removed-view-actions must be one "
                                   "of %s" % ", ".join(supportedJobActions))
    XML.SubElement(dsl, 'removedViewAction').text = removedViewAction

    supportedJobActions = ['JENKINS_ROOT', 'SEED_JOB']
    lookupStrategy = data.get('lookup-strategy', 'JENKINS_ROOT')
    if lookupStrategy not in supportedJobActions:
