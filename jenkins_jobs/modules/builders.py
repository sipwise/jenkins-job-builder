...
def openshift_build_verify(parser, xml_parent, data):
    ...
    :arg str name-space: If you run `oc get bc` for the project listed in
        "name-space", that is the value you want to put here. (default: test)
    ...
    XML.SubElement(osb, 'apiURL').text = str(
        data.get('api-url', 'https://openshift.default.svc.cluster.local'))
    XML.SubElement(osb, 'bldCfg').text = str(data.get("bld-cfg", 'frontend'))
    XML.SubElement(osb, 'nameSpace').text = str(
        data.get("name-space", 'test'))
...
def openshift_dep_verify(parser, xml_parent, data):
    ...
    XML.SubElement(osb, 'nameSpace').text = str(
        data.get("name-space", 'test'))
    XML.SubElement(osb, 'replicaCount').text = str(
        data.get("replica-count", 0))
    XML.SubElement(osb, 'authToken').text = str(data.get("auth-token", ""))
...
def openshift_img_tagger(parser, xml_parent, data):
    ...
    XML.SubElement(osb, 'apiURL').text = str(
        data.get('api-url', 'https://openshift.default.svc.cluster.local'))
    XML.SubElement(osb, 'testTag').text = str(
        data.get("test-tag", 'origin-nodejs-sample:latest'))
    XML.SubElement(osb, 'prodTag').text = str(
        data.get("prod-tag", 'origin-nodejs-sample:prod'))
    XML.SubElement(osb, 'nameSpace').text = str(
        data.get("name-space", 'test'))
    XML.SubElement(osb, 'authToken').text = str(data.get("auth-token", ""))
