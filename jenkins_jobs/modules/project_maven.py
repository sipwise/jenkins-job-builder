...
:Job Parameters:
    ...
    * **global-settings** (`str`): Path to custom maven global settings file
      It is possible to provide a ConfigFileProvider settings file as well
      org.jenkinsci.plugins.configfiles.maven.GlobalMavenSettingsConfig0123456789012
      (optional)
...
class Maven(jenkins_jobs.modules.base.Base):
    ...
    def root_xml(self, data):
        ...
        if 'settings' in data['maven']:
            ...
            if settings_file.startswith(
                ...
                settings = XML.SubElement(xml_parent, 'settings',
                              {'class':
                               self.settings['config-file-provider-settings']})
                XML.SubElement(settings, 'settingsConfigId').text = settings_file
            else:
                settings = XML.SubElement(xml_parent, 'settings',
                                          {'class':
        ...
        if 'global-settings' in data['maven']:
            # Support for Config File Provider
            global_settings_file = str(data['maven'].get('global-settings', ''))
            if global_settings_file.startswith(
                ...
                XML.SubElement(settings,'settingsConfigId').text = global_settings_file
            else:
                settings = XML.SubElement(xml_parent, 'globalSettings',
                                          {'class':
