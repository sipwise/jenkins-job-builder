...
def bitbucket_scm(xml_parent, data):
    ...
    :arg bool checkout-over-ssh: Checkout repo over ssh.
    :arg str checkout-over-ssh-credentials: The credentials to use for the
        checkout of the repo over ssh. (required when `checkout-over-ssh` is
        true)
    :arg bool filter-by-name-wildcard: Enable filter by name with wildcards.
        Requires the :jenkins-wiki:`SCM API Plugin <SCM+API+Plugin>`.
    :arg str filter-by-name-wildcard-include: Space-separated list of name
        patterns to consider. You may use * as a wildcard;
        for example: `master release*`
    :arg str filter-by-name-wildcard-exclude: Name patterns to ignore even
        if matched by the includes list. For example: `release`
    ...
    if data.get('local-branch', False):
        ...
        lbr_local = XML.SubElement(lbr_extension,
            'localBranch').text = "**"

    if data.get('checkout-over-ssh', False):
        cossh = XML.SubElement(traits,
            'com.cloudbees.jenkins.plugins.bitbucket.SSHCheckoutTrait')
        cossh_credentials = [
            ('checkout-over-ssh-credentials', 'credentialsId', ''),
        ]
        helpers.convert_mapping_to_xml(
            cossh, data, cossh_credentials, fail_required=True)

    if data.get('filter-by-name-wildcard'):
        wscmf_name = XML.SubElement(traits,
            'jenkins.scm.impl.trait.WildcardSCMHeadFilterTrait', {
            'plugin': 'scm-api',
        })
        wscmf_name_options = [
            ('filter-by-name-wildcard-include', 'includes', ''),
            ('filter-by-name-wildcard-exclude', 'excludes', ''),
        ]
        helpers.convert_mapping_to_xml(
            wscmf_name, data, wscmf_name_options, fail_required=True)
