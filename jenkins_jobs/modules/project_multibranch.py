...
def bitbucket_scm(xml_parent, data):
    ...
    :arg list build-strategies: Provides control over whether to build a branch 
        ...
        discovered initially or a change from the previous revision has been detected. (optional)
        Refer to :func:`~build_strategies <build_strategies>` for documentation.
...
def build_strategies(xml_parent, data): 
    ...
    :arg list build-strategies: Definition of build strategies.
        ...
        * **tags** (dict): Builds tags (subject to a configurable tag age time window)
            * **ignore-tags-newer-than** (int) The number of days since the tag was created before it is eligible for automatic building. (optional)
            * **ignore-tags-older-than** (int) The number of days since the tag was created after which it is no longer eligible for automatic building. (optional)
        * **change-request** (dict): Builds change requests / pull requests
            * **ignore-target-only-changes** (bool) Ignore rebuilding merge branches when only the target branch changed. (optional)
        * **regular-branches** (bool): 	Builds regular branches whenever a change is detected. (optional)
    ...
    for bbs_list in data.get('build-strategies', None):
        if 'tags' in bbs_list:
            tags = bbs_list['tags']
            tags_elem = XML.SubElement(bbs, ''.join([basic_build_strategies, '.TagBuildStrategyImpl']),{
                'plugin': 'basic-branch-build-strategies',
            })

            ignore_tags_newer_than = -1
            if 'ignore-tags-newer-than' in tags and tags['ignore-tags-newer-than'] >= 0:
                ignore_tags_newer_than = str(tags['ignore-tags-newer-than'] * 86400000)
            XML.SubElement(tags_elem, 'atMostMillis').text = str(ignore_tags_newer_than) 

            ignore_tags_older_than = -1
            if 'ignore-tags-older-than' in tags and tags['ignore-tags-older-than'] >= 0:
                ignore_tags_older_than = str(tags['ignore-tags-older-than'] * 86400000)
            XML.SubElement(tags_elem, 'atLeastMillis').text = str(ignore_tags_older_than) 

        if 'regular-branches' in bbs_list:
            XML.SubElement(bbs, ''.join([basic_build_strategies, '.BranchBuildStrategyImpl']),{
                'plugin': 'basic-branch-build-strategies',
            })
            
        if 'change-request' in bbs_list:
            cr = bbs_list['change-request']
            cr_elem = XML.SubElement(bbs, ''.join([basic_build_strategies, '.ChangeRequestBuildStrategyImpl']),{
                'plugin': 'basic-branch-build-strategies',
            })
            itoc = cr.get('ignore-target-only-changes', False)
            XML.SubElement(cr_elem, 'ignoreTargetOnlyChanges').text = str(itoc)

        if 'named-branches' in bbs_list:
            named_branch_elem = XML.SubElement(bbs, ''.join([basic_build_strategies, '.NamedBranchBuildStrategyImpl']),{
                'plugin': 'basic-branch-build-strategies',
            })

            if bbs_list['named-branches']:

                filters = XML.SubElement(named_branch_elem, 'filters')

                for nb in bbs_list['named-branches'] :
                    if 'exact-name' in nb:
                        exact_name_elem = XML.SubElement(filters, ''.join([basic_build_strategies, '.NamedBranchBuildStrategyImpl', '_-ExactNameFilter']))
                        exact_name_mapping = [
                            ('name', 'name', ''),
                            ('case-sensitive', 'caseSensitive', False)
                        ]
                        helpers.convert_mapping_to_xml(
                            exact_name_elem, nb['exact-name'], exact_name_mapping, fail_required=False)

                    if 'regex-name' in nb:
                        regex_name_elem = XML.SubElement(filters, ''.join([basic_build_strategies, '.NamedBranchBuildStrategyImpl', '_-RegexNameFilter']))
                        regex_name_mapping = [
                            ('regex', 'regex', '^.*$'),
                            ('case-sensitive', 'caseSensitive', False)
                        ]
                        helpers.convert_mapping_to_xml(
                            regex_name_elem, nb['regex-name'], regex_name_mapping, fail_required=False)
                            
                    if 'wildcards-name' in nb:
                        wildcards_name_elem = XML.SubElement(filters, ''.join([basic_build_strategies, '.NamedBranchBuildStrategyImpl', '_-WildcardsNameFilter']))
                        wildcards_name_mapping = [
                            ('includes', 'includes', '*'),
                            ('excludes', 'excludes', '')
                        ]
                        helpers.convert_mapping_to_xml(
                            wildcards_name_elem, nb['wildcards-name'], wildcards_name_mapping, fail_required=False)
