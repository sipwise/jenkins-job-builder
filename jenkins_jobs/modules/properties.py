...
def groovy_label(registry, xml_parent, data):
    ...
    :arg str script: Groovy script (default '') (requuired)
    ...
    .. literalinclude:: /../../tests/properties/fixtures/groovy-label-minimal.yaml
       :language: yaml
    ...
    secure = XML.SubElement(sub_element, 'secureGroovyScript')
    secure.set('plugin', 'script-security')
    mapping = [
        ('script', 'script', ''),
    ...
    helpers.convert_mapping_to_xml(
        secure, data, mapping, fail_required=True)
    if data is not None and 'classpath' in data:
        classpath = XML.SubElement(secure, 'classpath')
