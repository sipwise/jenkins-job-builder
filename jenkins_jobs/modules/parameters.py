...
def label_param(registry, xml_parent, data):
    ...
    :arg str matching-label: to run all nodes matching label
        'success', 'unstable' or 'all' (optional)
    ...
    if matching_label not in matching_label_dict:
        raise InvalidAttributeError(matching_label,
                                    matching_label, matching_label_dict.keys())

    XML.SubElement(pdef, 'triggerIfResult').text = \
        matching_label_dict[matching_label]

    eligibility_label = data.get('node-eligibility', 'all').lower()
    eligibility_label_dict = {'all':
                              'org.jvnet.jenkins.plugins.nodelabelparameter.'
                              'node.AllNodeEligibility',
                              'ignore-offline':
                              'org.jvnet.jenkins.plugins.nodelabelparameter.'
                              'node.IgnoreOfflineNodeEligibility',
                              'ignore-temp-offline':
                              'org.jvnet.jenkins.plugins.nodelabelparameter.'
                              'node.IgnoreTempOfflineNodeEligibility'
                              }
    ...
    XML.SubElement(pdef, 'nodeEligibility')\
        .set("class", eligibility_label_dict[eligibility_label])
