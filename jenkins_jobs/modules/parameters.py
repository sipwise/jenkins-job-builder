...
def label_param(registry, xml_parent, data):
    ...
    convert_mapping_to_xml(pdef, data, mapping, fail_required=False)

    for options in mapping:
        if options[0] == matching_label:
            XML.SubElement(pdef, 'triggerIfResult').text = options[1]

    eligibility_label = data.get('node-eligibility', 'all').lower()
    mapping = [
        ('all', 'org.jvnet.jenkins.plugins.'
                'nodelabelparameter.node.AllNodeEligibility', None),
        ('ignore-offline', 'org.jvnet.jenkins.plugins.'
                           'nodelabelparameter.node.'
                           'IgnoreOfflineNodeEligibility', None),
        ('ignore-temp-offline', 'org.jvnet.jenkins.plugins.'
                                'nodelabelparameter.node.'
                                'IgnoreTempOfflineNodeEligibility', None),
    ]
    convert_mapping_to_xml(pdef, data, mapping, fail_required=False)

    for options in mapping:
        if options[0] == eligibility_label:
