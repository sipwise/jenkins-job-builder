...
:jenkins-wiki:`Delivery Pipeline Plugin <delivery+pipeline+plugin>`.
...
:View Parameters:
    ...
    * **sorting** (`str`): How to sort the pipelines in the current view. Only
      ...
      Possible values are:
        * **none**: Default value.
        * **title**: Sort by title.
        * **failed_last_activity**: Sort by failed pipelines, then by last
          activity.
...
Example:
    ...
    .. literalinclude::
        /../../tests/views/fixtures/view_delivery_pipeline001.yaml

    .. literalinclude::
        /../../tests/views/fixtures/view_delivery_pipeline002.yaml
"""
        ...
        if len(components):
            ...
            for c in components:
                component_spec = XML.SubElement(
                    component_specs,
                    "se.diabol.jenkins.pipeline.DeliveryPipelineView" "_-ComponentSpec",
                )
        ...
        if len(regexps):
            ...
            for r in regexps:
                regexp_first_job = XML.SubElement(
                    regexp_first_jobs,
                    "se.diabol.jenkins.pipeline.DeliveryPipelineView" "_-RegExpSpec",
                )
