...
def postbuildscript(parser, xml_parent, data):
    ...
    mark_unstable_xml = XML.SubElement(pbs_xml, 'markBuildUnstable')
    if 'mark-unstable-if-failed' in data:
        mark_unstable_xml.text = str(data.get('mark-unstable-if-failed',
                                        False)).lower()
