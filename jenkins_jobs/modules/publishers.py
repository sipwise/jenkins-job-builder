...
def cloudformation(parser, xml_parent, data):
    ...
    delete_cloudformation = XML.SubElement(xml_parent, 'com.syncapse.'
                                                       'jenkinsci.plugins.'
                                                       'awscloudformation'
                                                       'wrapper.Cloud'
                                                       'FormationNotifier')
    delete_stacks = XML.SubElement(delete_cloudformation, 'stacks')
    for delete_stack in data.get('delete-stacks', []):
        step = XML.SubElement(delete_stacks, 'com.syncapse.jenkinsci.plugins.'
                                             'awscloudformationwrapper.'
                                             'SimpleStackBean')
        XML.SubElement(step, 'stackName').text = delete_stack['name']
        XML.SubElement(step, 'awsAccessKey').text = delete_stack['access-key']
        XML.SubElement(step, 'awsSecretKey').text = delete_stack['secret-key']
        prefix = str(delete_stack.get('prefix', False)).lower()
        XML.SubElement(step, 'isPrefixSelected').text = prefix
        region = delete_stack['region']
        if region not in region_dict:
            raise JenkinsJobsException("Region entered: " + region + " is not "
                                       "valid, must be one of: %s" %
                                       ", ".join(region_dict.keys()))
