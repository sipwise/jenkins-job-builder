...
def text_finder(parser, xml_parent, data):
    """yaml: text_finder
    This plugin lets you search keywords in the files you specified and additionally check builds' status
    ...
    XML.SubElement(finder, 'alsoCheckConsoleOutput').text = str(data.get('alsoCheckConsoleOutput', 'false')).lower()
    XML.SubElement(finder, 'succeedIfFound').text = str(data.get('succeedIfFound', 'false')).lower()
    XML.SubElement(finder, 'unstableIfFound').text = str(data.get('unstableIfFound', 'false')).lower()
