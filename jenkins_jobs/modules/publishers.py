...
def slack(parser, xml_parent, data):
    ...
    if 'team-domain' not in data:
            raise JenkinsJobsException('team-domain parameter is missing')
    XML.SubElement(slack, 'teamDomain').text = data['team-domain']

    if 'auth-token' not in data:
            raise JenkinsJobsException('auth-token parameter is missing')
    ...
    if 'room' not in data:
            raise JenkinsJobsException('room parameter is missing')
