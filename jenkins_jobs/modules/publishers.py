...
def junit(parser, xml_parent, data):
    ...
    :arg str health-report-amplification-factor: Amplification factor to apply
      to test failures when computing the test result contribution to the
      build health score. (default 1.0)
    ...
    XML.SubElement(junitresult, 'healthScaleFactor').text = str(
        data.get('health-report-amplification-factor', '1.0'))
