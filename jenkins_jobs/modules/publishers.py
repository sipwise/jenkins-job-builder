...
def plot(parser, xml_parent, data):
    ...
    :arg str numBuilds: number of builds to plot across
                        Leave empty to plot all builds.
    :arg str style:  Specifies the graph style of the plot
                     ...
                     Default to line.
    :arg str useDescr: When false (default), the X-axis labels are formed
                       using build numbers and dates, and the corresponding
                       tooltips contain the build descriptions.
                       When checked, the contents of the labels and tooltips
                       are swapped, with the descriptions used as X-axis
                       labels and the build number and date used for tooltips.
    :arg series: list data series definitions
        :arg str file: CSV files to include
        :arg str exclude: CSV files to exclude
                          Default to empty.
        :arg str url: for 'csv' and 'xml' file types
                      used when you click on a point
                      Default to empty.
        :arg str displayTable: for 'csv' file type
                               if true, original CSV will be shown above plot
                               Default to false.
        :arg: str label: used by 'properties' file type
                         Specifies the legend label for this data series.
                         Default to blank.
        :arg: str format: Type of file where we get datas.
                          Can be: properties, csv, xml
        :arg: str xpathType: The result type of the expression must be
                             supplied due
                             to limitations in the java.xml.xpath parsing.
                             The result can be: nodeset, boolean, string, or
                             number. Strings and numbers will be converted to
                             double. Boolean will be converted to 1 for true,
                             and 0 for false.
        :arg: str xpath: used by 'xml' file type
    ...
    for serie in series:
        ...
        if format_data == 'csv':
            ...
            XML.SubElement(subserie, 'displayTableFlag').text = \
                serie.get('displayTable', 'false')
        if format_data == 'xml':
            XML.SubElement(subserie, 'url').text = serie.get('url', '')
            XML.SubElement(subserie, 'xpathString').text = serie.get('xpath')
            xpathtype = serie.get('xpathType', 'NODE')
            if xpathtype not in xpath_list:
                raise Exception("XPath result entered is not valid, " +
                                "must be one of: NODESET, NODE, " +
    ...
    XML.SubElement(plugin, 'useDescr').text = data.get('useDescr', 'false')
    XML.SubElement(plugin, 'numBuilds').text = data.get('numBuilds', '')
    ...
    if style not in style_list:
        raise Exception("style entered is not valid, must be one of: " +
                        "area, bar, bar3d, line, line3d, stackedArea, " +
                        "stackedbar, stackedbar3d, waterfall")
