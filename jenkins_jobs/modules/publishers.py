...
def google_cloud_storage(parser, xml_parent, data):
    ...
    def expiring_elements(properties, upload_element, types):
        ...
        if 'bucket-name' not in properties:
            raise MissingAttributeError('bucket-name parameter is missing')
    ...
    for upload in uploads:
        for upload_type, properties in upload.items():
            ...
            if upload_type == valid_upload_types[0]:
                expiring_elements(properties, upload_element, types)
            elif upload_type == valid_upload_types[1]:
                build_log(properties, upload_element, types)
            elif upload_type == valid_upload_types[2]:
                classic(properties, upload_element, types)
            else:
