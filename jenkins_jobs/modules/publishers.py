...
def campfire(registry, xml_parent, data):
    ...
    if ('subdomain' in data and data['subdomain']):
        mapping.append(('subdomain', 'subdomain', None))
    if ('token' in data and data['token']):
        mapping.append(('token', 'token', None))
    if ('ssl' in data):
        mapping.append(('ssl', 'ssl', None))
    helpers.convert_mapping_to_xml(campfire, data, mapping, fail_required=True)
