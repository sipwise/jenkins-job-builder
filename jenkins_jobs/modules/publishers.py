...
def build_trends_publisher(plugin_name, xml_element, data):
    ...
    def append_thresholds(element, statuses, data, only_totals):
        ...
        for status in statuses:
            ...
            if only_totals is False:
                limits.append(('new-all', 'NewAll'))
                limits.append(('new-high', 'NewHigh'))
                limits.append(('new-normal', 'NewNormal'))
                limits.append(('new-low', 'NewLow'))
    ...
    for config_key, tag_name, default in settings:
        ...
        if config_key == 'thresholds':
            statuses = ['unstable', 'failed']

            append_thresholds(
                xml_config,
                statuses,
                data.get(config_key, default),
                data.get('dont-compute-new', True))
        else:
            config_value = data.get(config_key, default)
