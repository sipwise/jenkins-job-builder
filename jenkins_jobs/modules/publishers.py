...
def jabber(parser, xml_parent, data):
    ...
    if 'group-targets' in data:
        group_targets = data['group-targets']
        group_string = ' '.join(group_targets)
        gcimt = XML.SubElement(t, 'hudson.plugins.im.GroupChatIMMessageTarget')
        XML.SubElement(gcimt, 'name').text = group_string
        XML.SubElement(gcimt, 'notificationOnly').text = 'false'
    if 'individual-targets' in data:
        individual_targets = data['individual-targets']
        individual_string = ' '.join(individual_targets)
        dimt = XML.SubElement(t, 'hudson.plugins.im.DefaultIMMessageTarget')
        XML.SubElement(dimt, 'name').text = individual_string
