...
def trigger(parser, xml_parent, data):
    ...
    :arg str project: name of the job to trigger
    :arg str threshold: when to trigger the other job (default 'SUCCESS'), alternatives: SUCCESS, UNSTABLE, FAILURE
    ...
    if threshold not in thresholds.keys():
        raise Exception("threshold must be one of " + ", ".join(threshold.keys()))
    tname = XML.SubElement(tthreshold, 'name')
    tname.text = threshold
    tordinal = XML.SubElement(tthreshold, 'name')
