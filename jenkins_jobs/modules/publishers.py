...
def tap(parser, xml_parent, data):
    ...
    XML.SubElement(tap, 'failIfNoResults').text = \
        str(data.get('fail-if-no-results', False)).lower()

    XML.SubElement(tap, 'failedTestsMarkBuildAsFailure').text = \
        str(data.get('failed-tests-mark-build-as-failure', False)).lower()

    XML.SubElement(tap, 'outputTapToConsole').text = 'true'

    XML.SubElement(tap, 'enableSubtests').text = \
        str(data.get('enable-subtests', True)).lower()

    XML.SubElement(tap, 'discardOldReports').text = \
        str(data.get('discard-old-reports', False)).lower()

    XML.SubElement(tap, 'todoIsFailure').text = \
        str(data.get('todo-is-failure', True)).lower()
