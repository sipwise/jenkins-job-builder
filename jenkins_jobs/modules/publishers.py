...
def hockeyapp(registry, xml_parent, data):
    ...
    :arg list(dict) applications: list of applications to publish (at least
        ...
        :Application:
            ...
            * **notify-team** (`bool`) - Notify the team after upload (default false)
            * **download-allowed** (`bool`) - Whether to allow downloads (default false)
            * **upload** (`dict`) - Upload configuration

                :Upload:
                    * **type** (`str`) - Type of upload (possible: app, version)
                    * **app-id** (`str`) - App ID (types: version)

            * **release-notes** (`dict`) - Release notes configuration (default none)
    ...
    def configure_component(parent_xml, app, key, config_types, element_name, package_name):
        """Configures a component that has multiple types and where each type may
        ...
        if not config:
            ...
            for ct in config_types:
                ...
                raise JenkinsJobException("missing or invalid type for: %s" % key)
        ...
        if params_mapping:
            helpers.convert_mapping_to_xml(node, config, params_mapping, fail_required=True)
    ...
    for application in data['applications']:
        ...
        helpers.convert_mapping_to_xml(node, application, application_mapping, fail_required=True)

        configure_component(node, application, 'release-notes', release_notes_types,
                            'releaseNotesMethod', 'net.hockeyapp.jenkins.releaseNotes')

        configure_component(node, application, 'upload', upload_types,
                            'uploadMethod', 'net.hockeyapp.jenkins.uploadMethod')
