...
def warnings(parser, xml_parent, data):
    ...
    :arg bool use-previous-build-as-reference: If set the number of new warnings
        will always be computed based on the previous build, even if that build
        is unstable (due to a violated warning threshold). Otherwise the last
        ...
        should ensure that all new warnings will be finally fixed in
    ...
    if data.get('new-thresholds'):
        ...
        XML.SubElement(warnings, 'usePreviousBuildAsReference').text = \
            str(use_previous_build).lower()
