...
def checkstyle(parser, xml_parent, data):
    ...
    :arg str pattern: report filename pattern
    :arg bool canRunOnFailed: also runs for failed builds (instead of just stable or unstable builds)
    :arg bool shouldDetectModules:
    :arg int healthy: sunny threshold
    :arg int unHealthy: stormy threshold
    :arg str healthThreshold: Threshold priority for health status (high: only high, normal: high and normal, low: all)
    :arg dict thresholds
        :arg dict unstable
            :arg int totalAll
            :arg int totalHigh
        ...
        :arg dict failed
            ...
            :arg int totalNormal
            :arg int totalLow
    :arg str defaultEncoding: encoding for parsing or showing files (empty will use platform default)
    ...
    XML.SubElement(checkstyle, 'thresholdLimit').text = data.get('healthThreshold', 'low')

    XML.SubElement(checkstyle, 'pluginName').text = '[CHECKSTYLE] '

    XML.SubElement(checkstyle, 'defaultEncoding').text = data.get('defaultEncoding', '')
