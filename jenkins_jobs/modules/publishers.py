...
def tap(parser, xml_parent, data):
    ...
    p = XML.SubElement(xml_parent, 'org.tap4j.plugin.TapPublisher')

    XML.SubElement(p, 'testResults').text = data['results']

    XML.SubElement(p, 'failIfNoResults').text = \
        data.get('fail-if-no-results', False)

    XML.SubElement(p, 'failedTestsMarkBuildAsFailure').text = \
        data.get('failed-tests-mark-build-as-failure', False)

    XML.SubElement(p, 'outputTapToConsole').text = True

    XML.SubElement(p, 'enableSubtests').text = \
        data.get('enable-subtests', True)

    XML.SubElement(p, 'discardOldReports').text = \
        data.get('discard-old-reports', False)

    XML.SubElement(p, 'todoIsFailure').text = data.get('todo-is-failure',
                                                       True)
