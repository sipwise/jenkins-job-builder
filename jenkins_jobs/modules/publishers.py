...
def hockeyapp(parser, xml_parent, data):
    ...
    :arg str file-path: The path, relative to the build's workspace, to the generated .ipa (iOS), .app.zip (MacOS), or .apk (Android) file.
    :arg str symbols: An optional path, relative to the build's workspace, to the generated dSYM.zip (iOS and MacOS) or mapping.txt (Android) file.
    :arg str packed-libraries: Optional path, relative to the build's workspace, to an archive with generated .so files. This archive must have the following structure: ${ARCHITECTURE}/*.so
    :arg dict release-notes:
        ...
        :arg dict from-file:
            :from-file:
                ...
                * **interpret-release-notes-as-markdown** (`bool`) -- If checked HockeyApp expectes the provided Release notes to be written in Markdown.
        :arg dict input:
            :input:
                * **rel-notes-text** (`str`)
                * **interpret-release-notes-as-markdown** (`bool`) -- If checked HockeyApp expectes the provided Release notes to be written in Markdown.

    :arg bool allow-downloads: Enable if you want new versions to be available for download and installation immediately, without having to enable this manually in HockeyApp. (false)
    :arg str restrict-downloads-to-tags: By entering a comma-separated list of tags here, you can restrict downloads of your app to only users/devices with these tags. (default '')
    :arg bool notify-team: Enable if you want HockeyApp to notify your team about each uploaded version. (false)
    :arg str delete-old-versions: Number of old versions to keep on HockeyApp. (default '')
    :arg bool enable-debug-mode: Enables debug mode. (false)
    :arg bool fail-gracefully: If the upload to HockeyApp fails, the build is not marked as failed once fail gracefully is enabled. (false)
    :arg str custom-hockeyapp-api-url: Please insert an alternate Host-URL for the Hockey-App Service. (default '')
    ...
    recorder = XML.SubElement(xml_parent,
                            'hockeyapp.HockeyappRecorder')
    applications = XML.SubElement(recorder, 'applications')
    hockeyapp = XML.SubElement(applications, 'hockeyapp.HockeyappApplication')
    ...
    if 'upload-app' in data and 'app-id' in data:
        raise JenkinsJobsException("You cannot specify both app and version methods")
    if not 'app-id' in data:
        XML.SubElement(
            hockeyapp,
            'uploadMethod',
        ...
        XML.SubElement(method, 'appId').text = str(
            data.get('app-id', ''))
    # filePath
    if not 'file-path' in data:
        raise JenkinsJobsException("You have not specified file-path parameter.")
    ...
    if 'release-notes' in data:
        try:
            ...
            raise JenkinsJobsException("You have to specify at least one option for release-notes.")

        rdata = data['release-notes']
        if 'no-release-notes' in rdata and ('use-change-log' in rdata or 'from-file' in rdata or 'input' in rdata):
            raise JenkinsJobsException("You cannot specify more than one release-notes options.")
        if 'use-change-log' in rdata and ('no-release-notes' in rdata or 'from-file' in rdata or 'input' in rdata):
            raise JenkinsJobsException("You cannot specify more than one release-notes options.")
        if 'no-release-notes' in rdata:
            ...
            for key in nrdata:
                if 'filename' in key:
                    ...
                    mandatory = 1
                if 'interpret-release-notes-as-markdown' in key:
                    XML.SubElement(relnotes, 'isMarkdown').text = str(
                        key.get('interpret-release-notes-as-markdown', False)).lower()
            if mandatory != 1:
                raise JenkinsJobsException("filename must be defined when from-file option is present.")
        elif 'input' in rdata:
            relnotes = XML.SubElement(
                hockeyapp,
                'releaseNotesMethod',
            ...
            for key in nrdata:
                if 'rel-notes-text' in key:
                    XML.SubElement(relnotes, 'input').text = str(
    ...
    XML.SubElement(recorder, 'baseUrl').text = str(
        data.get('custom-hockeyapp-api-url', ''))
