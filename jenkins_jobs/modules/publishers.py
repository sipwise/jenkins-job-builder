...
def sounds_notifier(registry, xml_parent, data):
    ...
    def _check_required_values(sound_event):
        if sound_event['sound-id'] is None:
            raise MissingAttributeError('sound-id')
        if sound_event['to-result'] is None:
            raise MissingAttributeError('to-result')
        if sound_event['from-results'] is None:
            raise MissingAttributeError('from-results')

    def _create_sound_event_xml_tag(sound_events_xml):
        return XML.SubElement(sound_events_xml,
    ...
    if 'sound-events' in data and data['sound-events'] is not None:
        for sound_event in data.get('sound-events', []):
            _check_required_values(sound_event)

            sound_event_xml = _create_sound_event_xml_tag(sound_events_xml)
