...
def trigger_parameterized_builds(parser, xml_parent, data):
    ...
    Use of the `node-label-name` or `node-label-node` parameters
    ...
    :arg str node-label-name: Specify the Name for the NodeLabel parameter.
      (optional)
    :arg str node-label-node: Specify the Node for the NodeLabel parameter.
      (optional)
      ...
      publishers:
        - trigger-parameterized-builds:
            ...
            - project: yet_another_job_3
              node-label-node: node-label-foo
    ...
    for project_def in data:
        ...
        if ('predefined-parameters' in project_def
            ...
            or 'node-label-node' in project_def):
            ...
            if 'node-label-name' in project_def or \
               'node-label-node' in project_def:
                params = XML.SubElement(tconfigs,
                ...
                if 'node-label-node' in project_def:
                    label.text = project_def['node-label-node']
        else:
