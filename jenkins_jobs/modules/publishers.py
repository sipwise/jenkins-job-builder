...
def tasks(registry, xml_parent, data):
    ...
    :arg list files-to-scan: Fileset includes setting that specifies the
        ...
        the fileset is the workspace root. If no value is set, then the default
        ``**/*.java`` is used.
    :arg list files-to-exclude: Fileset excludes setting that specifies the
        ...
        files. Basedir of the fileset is the workspace root.
    ...
    :arg bool ignore-case: Ignore the case of the the tag identifiers.
    ...
    :arg str health-priorities: Determines which warning priorities should be
        considered when evaluating the build health. Can be ``high``,
        ``normal`` or ``low`` (default).
    :arg dict status-thresholds:
        :status-thresholds: Configure the build status and health. If the
            number of total or new warnings is greater than one of these
            thresholds then a build is considered as unstable or failed,
            respectively. I.e., a value of 0 means that the build status is
            changed if there is at least one warning found. Leave this field
            empty if the state of the build should not depend on the number of
            warnings. Note that for new warnings, you need to enable the next
    ...
    :arg bool use-prev-build-as-ref: If set the number of new warnings will
        ...
        reference. It is recommended to uncheck this option if the plug-in
        should ensure that all new warnings will be finally fixed in subsequent
        builds. Depends on ``compute-new-warnings`` option.
    :arg bool only-use-stable-as-ref: Use the last stable build as the
        ...
        considered. Depends on ``compute-new-warnings`` option.
    :arg str default-encoding: Default encoding when parsing or showing files.
