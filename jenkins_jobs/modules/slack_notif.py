...
class Slack(jenkins_jobs.modules.base.Base):
    ...
    def _load_global_data(self):
        ...
        """
        if not self.auth_token or not self.team_domain:
            relevant_entries = ('auth_token', 'team_domain')

            for config_entry in relevant_entries:
                # Retrieve the configuration information.
                try:
                    config_value = self.registry.global_config.get(
                        'slack', config_entry
                    )
                except (configparser.NoSectionError, ), e:
                    # Log the incident and afterwards reraise.
                    logger.fatal('The configuration file needs a slack section'
                                 ' containing: %s\n%s' % (config_entry, e)
                                 )
                    raise e

                # Ensure the value is defined and "sane"
                if not config_value or config_value == '':
                    raise jenkins_jobs.errors.JenkinsJobsException(
                        '%s must not be an empty string!' % config_entry
                    )

                # Finally attach the value to the instance.
                setattr(self, config_entry, config_value)

            self.build_server_url = self.registry.global_config.get(
                'jenkins', 'url'
            )
    ...
    def gen_xml(self, parser, xml_parent, data):
        ...
        XML.SubElement(slackpub, 'teamDomain').text = self.team_domain
        XML.SubElement(slackpub, 'buildServerUrl').text = self.build_server_url
        XML.SubElement(slackpub, 'authToken').text = self.auth_token
