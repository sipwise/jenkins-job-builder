...
class Slack(jenkins_jobs.modules.base.Base):
    ...
    def _load_global_data(self):
        ...
        if (not self.auth_token):
            try:
                self.auth_token = self.registry.global_config.get(
                    'slack', 'auth_token')
                # Require that the authtoken is non-null
                if not self.auth_token:
                    raise jenkins_jobs.errors.JenkinsJobsException(
                        "Slack auth_token must not be a blank string")
            except (configparser.NoSectionError,
                    jenkins_jobs.errors.JenkinsJobsException) as e:
                logger.fatal("The configuration file needs a slack section" +
                             " containing auth_token:\n{0}".format(e))
                raise e
            self.team_domain = None
            try:
                self.team_domain = self.registry.global_config.get(
                    'slack', 'team_domain')
                # Require that the team_domain is non-null
                if not self.team_domain:
                    raise jenkins_jobs.errors.JenkinsJobsException(
                        "Slack team_domain must not be a blank string")
            except (configparser.NoSectionError,
                    jenkins_jobs.errors.JenkinsJobsException) as e:
                logger.fatal("The configuration file needs a slack section" +
                             " containing team_domain:\n{0}".format(e))
                raise e
            self.build_server_url = self.registry.global_config.get('jenkins',
                                                                    'url')
