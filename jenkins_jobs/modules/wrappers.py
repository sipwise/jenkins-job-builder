...
def logfilesize(parser, xml_parent, data):
    ...
    :arg int size: Abort the build if logfile size is bigger than this
        value (in MiB). Only required in case set-own is true.
    ...
    set_own = data.get('set-own')
    if not isinstance(set_own, bool):
        raise JenkinsJobsException(
            "set-own must be bool type, received data '" +
            str(set_own) + "' of type=" + str(type(set_own)))
