...
def config_file_provider(parser, xml_parent, data):
    ...
    :arg list files: List of managed config files each denoted by **file**\
      each file can take three parameters

        :Parameter: * **file-id** (`str`)\
        The identifier for the managed config file
        :Parameter: * **target** (`str`)\
        Optionally defines where the file should be created
        :Parameter: * **variable** (`str`)\
        Optionally Defines an environment variable to be used
