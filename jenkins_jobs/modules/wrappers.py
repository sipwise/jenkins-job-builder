...
def docker_custom_build_env(parser, xml_parent, data):
    ...
    :arg string image-type: Docker image type. Valid values and their
        additional attributes described in the image_types_ table.
    :arg string docker-tool: The name of the docker installation to use
        (default 'Default')
    :arg string host: URI to the docker host you are using
    :arg string credentials-id: Argument to specify the ID of credentials
        to use for docker host (optional)
    :arg string registry-credentials-id: Argument to specify the ID of
    ...
    :arg string group: The user to run build has to be the same as the Jenkins
        slave user so files created in workspace have adequate owner and
        permission set
    :arg string command: Container start command (default '/bin/cat')
    :arg string net: Network bridge (default 'bridge')
    ...
    dockerfile         Build docker image from a Dockerfile in project
                       workspace. With this option, project can define the build
                       environment as a Dockerfile stored in SCM with project
                       source code

                         :context-path: (str) Path to docker context
                           (default '.')
                         :dockerfile: (str) Use an alternate Dockerfile to build
                           the container hosting this build
                           (default 'Dockerfile')
    ...
    .. literalinclude:: /../../tests/wrappers/fixtures/docker-custom-build-env001.yaml
