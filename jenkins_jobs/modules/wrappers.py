...
def vault(registry, xml_parent, data):
    ...
    :arg list secrets: List of secrets
        ...
        :secrets:
            * **secret-path** (`str`) -- The path of the secret in the vault server (required).
    ...
    conf_mapping = [
        ('vault-url', 'vaultUrl', None),
        ('credentials-id', 'vaultCredentialId', None)]
    convert_mapping_to_xml(configuration, data, conf_mapping, fail_required=True)

    secretsobj = XML.SubElement(vault, 'vaultSecrets')
    secrets = data.get('secrets', [])
    for secret in secrets:
        secretobj = XML.SubElement(secretsobj, 'com.datapipe.jenkins.vault.model.VaultSecret')
        XML.SubElement(secretobj , 'path').text = secret['secret'].get('secret-path', '')
        secretvaluesobj = XML.SubElement(secretobj, 'secretValues')
        for secretvalue in secret['secret']['secret-values']:
            secretvalueobj = XML.SubElement(secretvaluesobj, 'com.datapipe.jenkins.vault.model.VaultSecretValue')
            XML.SubElement(secretvalueobj , 'envVar').text = secretvalue['secret-value'].get('env-var', '')
            XML.SubElement(secretvalueobj , 'vaultKey').text = secretvalue['secret-value'].get('vault-key', '')
