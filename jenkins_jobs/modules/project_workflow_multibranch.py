...
class WorkflowMultiBranch(jenkins_jobs.modules.base.Base):
    ...
    def root_xml(self, data):
        ...
        periodic_trigger_map = collections.OrderedDict([
            ...
            ("2w", ("H H * * *", '1209600000')),
            ("4w", ("H H * * *", '2419200000'))])
        periodic_folder_trigger_value = project_def.get('periodic-folder-trigger')
        if periodic_folder_trigger_value:
            print(periodic_folder_trigger_value)
            if not periodic_trigger_map.get(periodic_folder_trigger_value):
                raise InvalidAttributeError(
                    'periodic-folder-trigger',
                    periodic_folder_trigger_value,
            ...
            XML.SubElement(periodic_folder_trigger, 'spec').text = \
                periodic_trigger_map[periodic_folder_trigger_value][0]
            XML.SubElement(periodic_folder_trigger, 'interval').text = \
                periodic_trigger_map[periodic_folder_trigger_value][1]
