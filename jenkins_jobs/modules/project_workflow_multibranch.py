...
doubling them in the script: ``{`` -> ``{{`` , otherwise it will be interpreted
by the python `str.format()` command.
                ...
                if 'github' in scm:
                    ...
                    if scm_data.get('discover-pr-forks-strategy', None):
                        ...
                        strategy = {'merge-current': '1',
                                    'current': '2',
                                    'both': '3'}
                        XML.SubElement(e, 'strategyId').text = strategy[scm_data['discover-pr-forks-strategy']]

                        trust = scm_data.get('discover-pr-forks-trust', 'contributors')
                        trust_map = {
                            'contributors': 'org.jenkinsci.plugins.github_branch_source.ForkPullRequestDiscoveryTrait$TrustContributors',  # noqa
                            'everyone': 'org.jenkinsci.plugins.github_branch_source.ForkPullRequestDiscoveryTrait$TrustEveryone',
                            'permission': 'org.jenkinsci.plugins.github_branch_source.ForkPullRequestDiscoveryTrait$TrustPermission',
                            'nobody': 'org.jenkinsci.plugins.github_branch_source.ForkPullRequestDiscoveryTrait$TrustNobody'
                        }
                        XML.SubElement(e, 'trust').attrib['class'] = trust_map[trust]
