...
:Job Parameters:
    ...
    * **includes** (`str`): Which branches should be
        included. (defaults to *, all)  (optional) be excluded. (defaults
        to empty, none)  (optional)
    * **bitbucket-server-url** (`str`) Bitbucket server URL, to be set when used 
        with Bitbucket Server. (defaults to empty, none) (optional)
    * **auto-register-webhook** (`bool`) Register webhook in Bitbucket.  
        (defaults to false) (optional) 
    * **ssh-port** (`int`) Bitbucket ssh port. (defaults to empty, none) (optional)
    * **repository-type** (`str`) Bitbucket repo type. (defaults to 'GIT') (optional)


Job with inline script example:

.. literalinclude:: /../../tests/yamlparser/fixtures/project_workflow_multibranch_template001.yaml
