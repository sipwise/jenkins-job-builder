...
:Job Parameters:
    ...
    * **scm** (`str`): The SCM definition. Currently only git or github as SCM is supported, use this as sub-structure of scm.
    ...
    * **github** (`str`): Use this as sub-structure of scm when using github.
        ...
        * **scan-credentials-id** (`str`): The credentialsId to use to scan github.
        * **checkout-credentials-id** (`str`): The credentialsId to use to checkout from github.  (defaults to same as scan-credentials-id) (optional)
        * **includes** (`str`): Which branches should be included. (defaults to *, all)  (optional)
        * **excludes** (`str`): Which branches should be excluded. (defaults to empty, none)  (optional)
        * **api-uri** (`str`): The github api uri (for hosted/on-site github).  (optional)
        * **build-origin-branch** (`bool`) Build origin branches.  (defaults to true) (optional)
        * **build-origin-branch-with-pr** (`bool`) Build origin branches also filed as PRs.  (defaults to true) (optional)
        * **build-origin-pr-merge** (`bool`) Build origin PRs (merged with base branch).  (defaults to false) (optional)
        * **build-origin-pr-head** (`bool`) Build origin PRs (unmerged head).  (defaults to false) (optional)
        * **build-fork-pr-merge** (`bool`) Build fork PRs (merged with base branch).  (defaults to true) (optional)
        * **build-fork-pr-head** (`bool`) Build fork PRs (unmerged head).  (defaults to false) (optional)
    ...
    def root_xml(self, data):
        ...
        if 'scm' in project_def:
            ...
            if 'github' in scm:
                ...
                XML.SubElement(source, 'scanCredentialsId').text = git['scan-credentials-id']
                if 'checkout-credentials-id' in git:
                    XML.SubElement(source, 'checkoutCredentialsId').text = git['checkout-credentials-id']
                else:
                    XML.SubElement(source, 'checkoutCredentialsId').text = 'SAME'
                ...
                # plugin sets them to.
                for (attr, xml_attr, default) in [('build-origin-branch', 'buildOriginBranch', True),
                    ...
                    XML.SubElement(source, xml_attr).text = str(git.get(attr, default))
            elif 'git' in scm:
                git = scm['git']
                source = XML.SubElement(branch_source, 'source')
