...
:Job Parameters:
    ...
    * **timer-trigger** (`str`): The timer spec for when the jobs
      should be triggered.
    * **periodic-folder-trigger** (`dict`):
        * **spec** (`str`): The timer spec for when the
          repository should be checked for branches. (optional)
        * **interval** (`str`): Interval for when the folder
          should be checked.
    ...
    def root_xml(self, data):
        ...
        if 'periodic-folder-trigger' in project_def:
            ...
            sub_trigger_items.append(
                (periodic_folder_trigger, project_def['periodic-folder-trigger'],  # noqa
                 [('spec', 'spec', None),
