...
def artifactory_deployment_patterns(xml_parent, data):
    ...
    XML.SubElement(deployment_patterns, 'includePatterns').text = \
        ','.join(data.get('include-deployment-pattern', ['']))
    XML.SubElement(deployment_patterns, 'excludePatterns').text = \
        ','.join(data.get('exclude-deployment-pattern',
                          ['*password*,*secret*']))
...
def artifactory_env_vars_patterns(xml_parent, data):
    ...
    XML.SubElement(deployment_patterns, 'includePatterns').text = \
        ','.join(data.get('include-env-var-pattern', ['']))
    XML.SubElement(deployment_patterns, 'excludePatterns').text = \
        ','.join(data.get('exclude-env-var-pattern', ['*password*,*secret*']))
...
def artifactory_repository(xml_parent, data, target):
    if 'release' in target:
        XML.SubElement(xml_parent, 'keyFromText').text = \
            data.get('deploy-release-repo-key', '')
        XML.SubElement(xml_parent, 'keyFromSelect').text = \
            data.get('deploy-release-repo-key', '')
        XML.SubElement(xml_parent, 'dynamicMode').text = \
