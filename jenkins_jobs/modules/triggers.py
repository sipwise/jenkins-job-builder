...
def build_gerrit_triggers(xml_parent, data):
    ...
    trigger_on_events.attrib['class'] = 'linked-list'
...
def gerrit(parser, xml_parent, data):
    ...
    :arg list trigger-on: Events to react on (additive to the events
        below).
         ...
         * **comment-added-event** (`dict`) -- Trigger on comment added.
           ...
           :Comment added:
               * **approval-category** (`str`) -- Approval (verdict) category
                 ...
                 control
                 <http://gerrit.googlecode.com/svn/documentation/2.1/access-control.html#categories>`_
