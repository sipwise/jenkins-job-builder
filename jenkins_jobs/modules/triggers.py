...
def monitor_files(parser, xml_parent, data):
    ...
    :arg list files: List of files to monitor
      ...
      :File: * **path** (`str`) -- File Path to monitor. You can use a pattern
             ...
             * **check-content** (`list`) -- List of content changes of the
               ...
               :Content Nature: * **simple** (`bool`) -- Trigger on change in
                                  content of the specified file (whatever the
                                  type file). (default false)
                                * **jar** (`bool`) -- Trigger on change in
                                  content of the specified JAR file.
                                  (default false)
                                * **tar** (`bool`) -- Trigger on change in
                                  content of the specified Tar file.
                                  (default false)
                                * **zip** (`bool`) -- Trigger on change in
                                  content of the specified ZIP file.
                                  (default false)
                                * **source-manifest** (`list`) -- Trigger on
                                  change to MANIFEST files.

                                  :MANIFEST File: * **keys** (`list`) -- List
                                                    of keys to inspect.
                                                    (optional)
                                                  * **all-keys** (`bool`) - If
                                                    true, take into account all
                                                    keys. (default true)

                                * **jar-manifest** (`list`) -- Trigger on
                                  change to MANIFEST files (contained in jar
                                  files).
                                ...
                                * **properties** (`list`) -- Monitor the
                                  contents of the properties file.

                                  :Properties File: * **keys** (`list`) -- List
                                                      of keys to inspect.
                                                      (optional)
                                                    * **all-keys** (`bool`) --
                                                      If true, take into
                                ...
                                * **xml** (`list str`) -- Trigger on change to
                                  the listed XPath expressions.
                                * **text** (`list str`) -- Trigger on change to
                                  the listed regular expressions.
    ...
    .. literalinclude:: /../../tests/triggers/fixtures/monitor-files001.yaml
       :language: yaml
    .. literalinclude:: /../../tests/triggers/fixtures/monitor-files002.yaml
       :language: yaml
    """
    ...
    has_key_content_types = ['source-manifest', 'jar-manifest', 'properties']
    ...
    for file_info in files:
        ...
        for content in check_content:
            ...
            content_type = valid_content_types.get(type_name)
            if type_name in has_key_content_types:
