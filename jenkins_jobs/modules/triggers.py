...
def gerrit_handle_legacy_configuration(data):
    ...
    def convert_dict(d, old_keys):
        for old_key in old_keys:
            if old_key in d:
                ...
                logger.warning("'%s' is deprecated and will be removed after "
                               "1.0.0, please use '%s' instead", old_key,
                               new_key)
                d[new_key] = d[old_key]
                del d[old_key]
    ...
    if old_format_events:
        logger.warning("The events: %s; which you used is/are deprecated. "
    ...
    for event in data.get('trigger-on', []):
        if isinstance(event, six.string_types):
            ...
            if event == 'patchset-uploaded-event':
                logger.warning("'%s' is deprecated. Use "
                               "'patchset-created-event' format instead.",
                               event)
...
def pollscm(parser, xml_parent, data):
    ...
    try:
        ...
        logger.warning("Your pollscm usage is deprecated, please use"
