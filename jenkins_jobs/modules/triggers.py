...
def build_pollurl_content_types(xml_parent, data):
    XML.SubElement(xml_parent, 'inspectingContent').text = str(
        bool(data)).lower()
    content_types = XML.SubElement(xml_parent, 'contentTypes')
...
def build_pollurl_urls(xml_parent, data):
    for url in data:
        entry = XML.SubElement(xml_parent,
        ...
        build_pollurl_content_types(entry, url.get('check-content', []))
...
def pollurl(parser, xml_parent, data):
    ...
    build_pollurl_urls(entries, data.get('urls', []))
