...
def build_result(parser, xml_parent, data):
    ...
    for group in data['groups']:
        ...
        if not group.get('jobs', []):
            raise jenkins_jobs.errors.JenkinsJobsException('Jobs is missing'
                                                           ' and a required'
                                                           ' element')
        ...
        for result in group.get('results', ['success']):
            if result not in rsult_dict:
                raise jenkins_jobs.errors.JenkinsJobsException('Result'
                                                               ' entered is'
                                                               ' not valid,'
                                                               ' must be one'
                                                               ' of: '
            ...
            model_checked = XML.SubElement(checked_results, 'org.jenkinsci.'
                                           ...
                                           'CheckedResult')
            XML.SubElement(model_checked, 'checked').text = rsult_dict[result]
