...
def gitlab_merge_request(registry, xml_parent, data):
    ...
    :arg string assignee-filter: only MRs with this assigned user will trigger the build automatically (default jenkins)
    :arg string tag-filter: only MRs with this label will trigger the build automatically (default Build)
    :arg string trigger-comment: force build if this comment is the last in merge reguest
    :arg string publish-build-progress-messages: publish build progress messages (except build failed)
    ...
    XML.SubElement(ghprb, '__assigneeFilter').text = data.get('assignee-filter')
    XML.SubElement(ghprb, '__tagFilter').text = data.get('tag-filter')
    XML.SubElement(ghprb, '__triggerComment').text = data.get('trigger-comment')
    XML.SubElement(ghprb, '__publishBuildProgressMessages').text = data.get('publish-build-progress-messages')
    XML.SubElement(ghprb, '__autoCloseFailed').text = data.get('auto-close-failed')
    XML.SubElement(ghprb, '__autoMergePassed').text = data.get('auto-merge-passed')
