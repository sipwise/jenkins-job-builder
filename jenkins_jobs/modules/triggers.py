...
def dockerhub_notification(registry, xml_parent, data):
    ...
    :arg bool specified-repositories: The specified repositories will
        trigger this job. (default false)

        :repositories: Specified repositories to trigger the job.
            (default [])
    ...
    if data.get('referenced-image') is True:
        XML.SubElement(option, 'org.jenkinsci.plugins.'
                       'registry.notification.'
                       'opt.impl.TriggerForAllUsedInJob')
    if data.get('specified-repositories') is True:
