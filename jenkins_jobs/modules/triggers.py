    ...
    for group in data['groups']:
        ...
        for result in group['results']:
            model_checked = XML.SubElement(checked_results, 'org.jenkinsci.'
                                           ...
                                           'CheckedResult')
            # Need upper().replace() to maintain backward compatibility with
            # previous versions where it used the old format '-'
            result = result.upper().replace('-', '_')
            XML.SubElement(model_checked, 'checked').text = result
            if result not in valid_results:
                raise InvalidAttributeError('results', result, valid_results)
