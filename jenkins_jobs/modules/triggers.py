...
def gerrit(parser, xml_parent, data):
    ...
    :arg str dynamic-trigger-url: if you specify this option, the gerrit
        trigger confguration will be fetched from there on a regular interval
    ...
    XML.SubElement(gtrig, 'noNameAndEmailParameters').text = str(
        data.get('no-name-and-email', 'true')).lower()
    dyn_trigger = str(data.get('dynamic-trigger-url', ''))
    if dyn_trigger != '':
        XML.SubElement(gtrig, 'dynamicTriggerConfiguration').text = 'true'
