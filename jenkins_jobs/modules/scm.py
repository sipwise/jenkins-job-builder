...
def repo(self, xml_parent, data):
    ...
    :arg str manifest-branch: The branch of the manifest to use, uses
             master if none set
    :arg str manifest-file: Initial manifest file to use when initialising,
             uses default.xml if none set
    :arg str manifest-group: Only retrieve those projects in the manifest
             tagged with the provided group name, use 'default' if not set
    :arg str destination-dir: Location relative to the workspace root to clone
             to, uses workspace if not set
    :arg str repo-url: custom url to retrieve the repo application, by default
             repo will be pulled from it's default location
    :arg str mirror-dir: Path to mirror directory to reference when
             initialising
    :arg int jobs: Number of projects to fetch simultaneously
    ...
    for elem in mapping:
        ...
        if optname and optname in data:
            val = data[optname]
        if not val and not elem[2]:
            continue
        xe = XML.SubElement(scm, xmlname, attrs)
        if type(elem[2]) == bool:
            xe.text = str(val).lower()
        elif type(elem[2]) == int:
            xe.text = str(val)
        else:
            xe.text = val
