...
remoteName/\*')
    ...
    :extensions:
        ...
        :arg dict clean:
            :clean:
                * **after** (`bool`) - Clean up the workspace after every
                    checkout by deleting all untracked files and directories,
                    including those which are specified in .gitignore. It also
                    resets all tracked files to their versioned state. This
                    ensures that the workspace is in the same state as if you
                    cloned and checked out in a brand-new empty directory, and
                    ensures that your build is not affected by the files
                    generated by the previous build.
                * **before** (`bool`) - Clean up the workspace before every
                    checkout by deleting all untracked files and directories,
                    including those which are specified in .gitignore. It also
                    resets all tracked files to their versioned state. This
                    ensures that the workspace is in the same state as if you
    ...
    for elem in mapping:
        ...
        if optname in submodule_cfgs:
            if any(i in data for i in submodule_cfgs):
                logger.warn('\'disable-submodules\' and '
                            '\'recursive-submodules\' configurations are '
                            'deprecated. Please use the \'submodule\' '
                            'section instead to configure these settings '
                            'instead.')
