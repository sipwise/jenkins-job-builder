...
def git(self, xml_parent, data):
    ...
    mapping = [
        ...
        ("use-author", 'hudson.plugins.git.extensions.impl.AuthorInChangelog', False),
        ("clean", 'hudson.plugins.git.extensions.impl.CleanCheckout', False),
        ("wipe-workspace", 'hudson.plugins.git.extensions.impl.WipeWorkspace', False),
        ("prune", 'hudson.plugins.git.extensions.impl.PruneStaleBranch', False),
        ("fastpoll", 'hudson.plugins.git.extensions.impl.DisableRemotePoll', True),
        ('skip-tag', 'hudson.plugins.git.extensions.impl.PerBuildTag', True)
    ]
    ...
    user_exclusion = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.UserExclusion')
    for user in data.get('excluded-users', []):
        XML.SubElement(user_exclusion, 'excludedUsers').text = user

    path_restriction = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.PathRestriction')
    if 'included-regions' in data:
        include_string = '\n'.join(data['included-regions'])
        XML.SubElement(path_restriction, 'includedRegions').text = include_string

    if 'excluded-regions' in data:
        exclude_string = '\n'.join(data['excluded-regions'])
        XML.SubElement(path_restriction, 'excludedRegions').text = exclude_string

    if 'merge' in data:
        merge = data['merge']
        prebuild_merge = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.PreBuildMerge')
        options = XML.SubElement(prebuild_merge, 'options')
        XML.SubElement(options, 'mergeRemote').text = merge.get('remote', 'origin')
    ...
    build_chooser_setting = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.BuildChooserSetting')
    build_chooser = XML.SubElement(build_chooser_setting, 'buildChooser', {'class': choosing_strategy})
    XML.SubElement(build_chooser, 'separator').text = '#'

    submodule_option = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.SubmoduleOption')
    XML.SubElement(submodule_option, 'disableSubmodules').text = str(data.get('disable-submodules', False)).lower()
    XML.SubElement(submodule_option, 'recursiveSubmodules').text = str(data.get('recursive-submodules', False)).lower()

    if 'basedir' in data:
        relative_target_dir = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.RelativeTargetDirectory')
        XML.SubElement(relative_target_dir, 'relativeTargetDir').text = data['basedir']

    clone_option = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.CloneOption')
    if 'reference-repo' in data:
        XML.SubElement(clone_option, 'reference').text = data['reference-repo']
    if data.get('shallow-clone', False):
        XML.SubElement(clone_option, 'shallow').text = str(data.get('shallow-clone', False)).lower()

    user_identity = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.UserIdentity')
    ...
    if 'scm-name' in data:
        scm_name = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.ScmName')
    ...
    if 'local-branch' in data:
        local_branch = XML.SubElement(xml_extensions, 'hudson.plugins.git.extensions.impl.LocalBranch')
