...
def tfs(self, xml_parent, data):
    ...
    **NOTE**: TFS Password must be entered manually on the project if a
    ...
    # TODO(pabelanger): In the future, with would be nice to have a place that
    # can pull passwords into JJB without having to commit them in plaintext.
    # This could also integrate nicely with global configuration options.
