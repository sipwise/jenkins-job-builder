...
def openshift_img_streams(parser, xml_parent, data):
    ...
    this baked-in polling mechanism provided by Jenkins is leveraged by
    exposing some of the common semantics between OpenShift ImageStreams
    (which are abstractions of Docker repositories)
    and SCMs - versions / commit IDs of related artifacts
    (images vs. programmatics files);
    Requires the Jenkins :<Openshift+Plugin - https://www.openshift.com/>`.

    :arg str image-stream-name: The name of the ImageStream is what shows up
                                in the NAME column if you dump all the
                                ImageStream's with the `oc get is` command
                                invocation.
                                (default: nodejs-010-centos7)
    ...
    :arg str name-space: The value here should be whatever was the output
                         form `oc project` when you created the BuildConfig
                         you want to run a Build on.
                        (default: test)
    ...
    .. literalinclude:: /../../tests/scm/fixtures/openshift_img_streams001.yaml
    """
