...
def git(parser, xml_parent, data):
    ...
    for elem in mapping:
        ...
        if optname in submodule_cfgs:
            if optname in data:
                logger.warning("'{0}' is deprecated, please convert to use "
                               "the 'submodule' section instead as support "
                               "for this top level option will be removed in "
                               "a future release.".format(optname))
    ...
    if 'clean' in data:
        ...
        if isinstance(data['clean'], bool):
            ...
            clean_before = False
            logger.warning("'clean: bool' configuration format is deprecated, "
                           "please use the extension style format to "
