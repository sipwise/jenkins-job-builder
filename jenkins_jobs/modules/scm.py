...
def bzr(parser, xml_parent, data):
    ...
    :arg bool clean_tree:
        Clean up the workspace (using bzr) before pulling the branch.
    :arg bool lightweight_checkout:
        Use a lightweight checkout instead of a full branch.
    ...
    mapping = [
        # option, xml name, default value (text), attributes (hard coded)
        ('url', 'source', ''),
        ('clean_tree', 'cleantree', False),
        ('lightweight_checkout', 'checkout', False),
    ]
    ...
    if browser not in browser_name_to_class:
        valid = sorted(browser_name_to_class.keys())
        raise JenkinsJobsException("Browser entered is not valid must be one "
                                   "of: %s or %s." % (", ".join(valid[:-1]),
                                                      valid[-1]))
