...
class JenkinsManager(object):
    ...
    def exists(self, job):
        exists = (self._jjb_config.builder['ignore_cache'] or
                  (self.cache.is_cached(job.name) and self.cache.data[job.name] != ''))
        if not exists:
            logger.debug("'{0}' does not currently exist in the cache".format(job.name))
        return exists
