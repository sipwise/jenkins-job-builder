...
import fnmatch
import six
from jenkins_jobs.errors import JenkinsJobsException, PluginInfoError
...
class ModuleRegistry(object):
    ...
    def cmp_plugin_version(self, plugin_long_name, version):
        """
        Given a plugin longName and a version string, compare the version of
        that plugin to the string version passed to this function.

        If plugin_info_version == version, return 0
        If plugin_info_version > version, return 1
        If plugin_info_version < version, return -1

        This function compares versions using tuples generated by
        pkg_resources.parse_version.
        """
        given_version = pkg_resources.parse_version(version)
        plugin_info = self.plugins_dict.get(plugin_long_name, None)

        if plugin_info is None:
            raise PluginInfoError("Plugin information not found for '{0}'".
                                  format(plugin_long_name))

        plugin_info_version = plugin_info.get('version')

        plugin_info_version = pkg_resources.parse_version(
            plugin_info['version'])

        if plugin_info_version == given_version:
            return 0
        elif plugin_info_version < given_version:
            return -1
        elif plugin_info_version > given_version:
            return 1
