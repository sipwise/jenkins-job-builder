...
class ModuleRegistry(object):
    ...
    def _get_plugins_info_dict(plugins_list):
        ...
        """
        def int_or_string(it):
            """ Maven version strings can sometimes consist of non-numeric
            strings which means we need to simply return the value passed
            rather than try to convert it to a string.
            """
            try:
                return int(it)
            except ValueError:
        ...
        def mutate_plugin_info(plugin_info):
            ...
            plugin_info. We assume that plugin version strings are valid Maven
            version strings
            """
            version = plugin_info.get('version', '0')
            # plugin_info['version'] = pkg_resources.parse_version(version)
            plugin_info['version'] = tuple(map(int_or_string,
                                               version.split('.')))
