...
class UpdateSubCommand(base.BaseSubCommand):
    ...
    def parse_args(self, subparser):
        ...
        update.add_argument(
            '-j', '--jobs-only',
            action='append_const',  # Use list to make the -j|-v exclusive
            dest='update',
            const='jobs',
    ...
    def execute(self, options, jjb_config):
        ...
        if options.update:
            if len(options.update) == 1:
                update = options.update[0]
            else:
                raise Exception("ERROR: Cannot use options --jobs-only|-j "
                                "and --views-only|-v together.")
        else:
            update = 'all'
