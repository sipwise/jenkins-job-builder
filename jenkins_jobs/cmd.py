...
const.DEFAULT_JENKINS_URL = "http://localhost:8080"
const.DEFAULT_USER = None
const.DEFAULT_PASSWORD = None
const.DEFAULT_IGNORE_CACHE = False
...
def main():
    ...
    parser.add_argument(
        ...
        dest='ignore_cache', default=const.DEFAULT_IGNORE_CACHE,
    ...
    if os.path.isfile(conf):
        ...
        logger.debug("set param for test run")
        ## to avoid the 'no section' and 'no option' errors when testing
        config.add_section("jenkins")
        config.set("jenkins", "url", const.DEFAULT_JENKINS_URL)
        config.set("jenkins", "user", const.DEFAULT_USER)
        config.set("jenkins", "password", const.DEFAULT_PASSWORD)
        config.set("jenkins", "ignore_cache", const.DEFAULT_IGNORE_CACHE)
    ...
    ignore_cache = const.DEFAULT_IGNORE_CACHE
