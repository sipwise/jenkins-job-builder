...
def create_parser():
    ...
    # subparser: get-plugins-info
    parser_get_plugins_info = subparser.add_parser(
        'get-plugins-info',
        help='Get plugins info yaml by querying Jenkins server.')
    parser_get_plugins_info.add_argument(
        '-o', '--output-file', dest='plugins_info_file',
...
def execute(options, config):
    ...
    elif options.command == 'get-plugins-info':
