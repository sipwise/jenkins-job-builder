...
class CmdTests(unittest.TestCase):
    ...
    fixtures_path = os.path.abspath(__file__).rsplit(os.path.sep, 1)[0] \
        + '/fixtures'
    tools_path = os.path.abspath(__file__).rsplit(os.path.sep, 3)[0] \
        + '/tools'
    ...
    def test_non_existing_job(self):
        ...
        args = self.parser.parse_args(['test',
                                       self.fixtures_path + '/cmd-001.yaml',
                                       'invalid'])
        config = ConfigParser.ConfigParser()
        config.readfp(cStringIO.StringIO(cmd.DEFAULT_CONF))
    ...
    def test_console_output(self):
        ...
        """
        command = 'python ' + self.tools_path + '/jenkins-jobs.py' \
            + ' test ' + self.fixtures_path + '/cmd-001.yaml'
        process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
        console_out = unicode(process.communicate()[0], "utf-8")
        xml_content = u"%s" % codecs.open(self.fixtures_path + '/cmd-001.xml',
                                          'r',
                                          'utf-8').read()
