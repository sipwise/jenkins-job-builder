...
class TestConfigs(CmdTestsBase):
    ...
    def test_use_global_config(self):
        ...
        self.assertEquals(cmd.getConfigFile(args), '/etc/jenkins_jobs/jenkins_jobs.ini')


    def test_use_config_in_user_home(self):
        """
        Verify that JJB uses config file in user home if nothing else available
        """
        args = self.parser.parse_args(['test', 'foo'])
        ...
        with patch('os.path.isfile', return_value=True):
            self.assertEquals(cmd.getConfigFile(args), expected_loc)
