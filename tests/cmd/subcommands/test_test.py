...
class TestJenkinsGetPluginInfoError(CmdTestsBase):
    """ This test class is used to for testing the 'test' subcommand when we
    want to validate its behavior when
    jenkins_jobs.builder.Jenkins.get_plugins_info is not mocked.
    """
    ...
    def test_console_output_jenkins_connection_failure_warning(
        ...
        with mock.patch('sys.stdout'):
            self.assertThat(cmd.main(['test', os.path.join(self.fixtures_path,
                            'cmd-001.yaml')]),
                            Not(Raises(MatchesException(error.URLError))))
