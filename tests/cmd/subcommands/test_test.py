...
class TestTests(CmdTestsBase):
    ...
    def test_fail_at_default_of_command_execution(self, update_job_mock):
        """
        Run test mode and fail at no retry as default of command execution
        with JenkinsException
        """
    ...
    def test_fail_at_retry_out_of_command_execution(self, update_job_mock):
        ...
        self.config.set('job_builder', 'cmd_sleep', '1')
        self.assertRaises(jenkins.JenkinsException, cmd.execute,
                          args, self.config)
    ...
    def test_fail_at_no_retry_of_command_execution(self, update_job_mock):
        ...
        args = self.parser.parse_args(['test',
                                       os.path.join(self.fixtures_path,
                                                    'cmd-001.yaml'),
                                       'foo-job'])
        self.config.set('job_builder', 'cmd_tries', '3')
        self.config.set('job_builder', 'cmd_sleep', '1')
