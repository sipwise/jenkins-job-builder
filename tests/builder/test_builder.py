...
class TestCaseTestBuilder(TestCase):
    ...
    @mock.patch.object(jenkins_jobs.builder.Jenkins, 'get_info',
                       side_effect=URLError(IOError(errno.ECONNREFUSED,
                                                    'doesnot',
                                                    'matter')))
    def test_wait_for_jenkins_conn_refused2(self, jenkins_mock):
        "Wait for a jenkins that does not yet listen (python-jenkins < 0.3.0)"
        self.assertEqual(self.builder.wait_for_jenkins(1), False)
    ...
    def test_wait_for_jenkins_failure2(self, jenkins_mock):
        "Bail out of a jenkins with a permanent error (python-jenkins < 0.3.0)"
