...
class TestCaseTestBuilder(TestCase):
     def setUp(self):
         self.builder = jenkins_jobs.builder.Builder(
              'http://jenkins.example.com', 'doesnot','matter')
         TestCase.setUp(self)
     ...
     @mock.patch.object(jenkins_jobs.builder.Jenkins,'get_info',
                        side_effect=urllib2.URLError(IOError(errno.ECONNREFUSED,
                                                             'doesnot',
                                                             'matter')))
     ...
     @mock.patch.object(jenkins_jobs.builder.Jenkins,'get_info',
                        side_effect=urllib2.URLError(IOError(errno.EPIPE)))
     def test_wait_for_jenkins_failure(self, jenkins_mock):
          """Bail out of a jenkins with a permanent error"""
          self.assertRaises(urllib2.URLError,  self.builder.wait_for_jenkins, 1)
