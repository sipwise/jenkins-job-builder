...
from jenkins_jobs.errors import YAMLStructureError
from tests.base import SingleJobTestCase
from tests.base import get_scenarios
from tests.base import mock


class TestCaseModuleMissingIndents(TestWithScenarios, TestCase,
                                   SingleJobTestCase):
    ...
    def test_yaml_snippet(self, mock_logger):
        ...
        if os.path.basename(self.in_filename).startswith("exception_"):
            with ExpectedException(YAMLStructureError):
                super(TestCaseModuleMissingIndents, self).test_yaml_snippet()
        else:
            super(TestCaseModuleMissingIndents, self).test_yaml_snippet()
