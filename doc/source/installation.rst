...
which will upload the job and view definitions to Jenkins if needed.  Jenkins Job
Builder maintains, for each host, a cache [#f1]_ of previously configured jobs
and views, so that you can run that command as often as you like, and it will
only update the configurations in Jenkins if the defined definitions has
...
Do the same to delete a list of views::

  jenkins-jobs delete-view Foo1 Foo2
...
.. program-output:: jenkins-jobs delete-view --help
