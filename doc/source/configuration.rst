...
Macros can takes parameter, letting you define a generic macro and more
specific one without having to duplicate code::

    # The 'add' macros takes a 'number' parameter and will create a
    # a job which echo 'Adding ' followed by the 'number' parameter.
    - builder:
        name: add
        builders:
...
As you can see, the specialized macro ``addtwo`` reused the definitions from
the generic macro ``add``.  Whenever you forget a a parameter from a macro,
